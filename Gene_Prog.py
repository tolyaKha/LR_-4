...
gene_quantity = 2 # количество генов, хромосома;максимальное количество преодолеваемых вершин без учета 
#начальной (start_top) и конечной (end_top)
...
for generation in range(generation_quantity - 1): # перебор поколений, первое поколение отдельно
  ...
  for k in population_1:
    ...
    # -------------------мутация гена-----------------------------
    individual_1[random.randint(1, gene_quantity)] = random.randint(0, top_quantity - 1) # присвоить произвольное значение гена 
    #из генофонда в произвольное место, за исключением начальной (start_top) и конечной (end_top) вершин-генов
